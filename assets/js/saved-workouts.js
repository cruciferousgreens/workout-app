/* ===== module: saved-workouts.js ===== */
    ...
    function renderWorkoutTemplateList() {
      ...
      const card=item=>{
        ...
        return `<div class="swipe-item saved-swipe">${rail}<div class="picker-item saved-workout-card swipe-content saved-workout-row"><button class="saved-workout-open" type="button" data-saved-id="${escapeHtml(item.id)}" aria-label="Open ${escapeHtml(item.name)}">${body}</button></div></div>`;
      };
      ...
      /* #260: the swipe rail's delete opens the standard confirmation modal
         (with sync tombstone) instead of deleting instantly. */
      host.querySelectorAll('.delete-saved-row').forEach(b=>b.addEventListener('click',()=>{
        const t=workoutState.templates.find(x=>x.id===b.dataset.savedId);if(!t||t.builtIn)return;
