...
/* ===== module: dashboard-stats.js ===== */
    ...
    function renderStats() {
      ...
      /* #319 (user 2026-09-13): the muscle map has its OWN Weight|Sets toggle
         (independent of the Weight-by-muscle list toggle below it). */
      ...
      $('#muscleStats').innerHTML=Object.keys(muscles).length?`<div class="tag-row">${Object.entries(muscles).sort((a,b)=>b[1]-a[1]).map(([m,n])=>musclePill(m,` · ${n} set${n===1?'':'s'}`)).join('')}</div>`:'<p class="section-note">Complete a workout to start building muscle-level stats.</p>';
