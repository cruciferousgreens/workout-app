...
/* ===== module: programs.js ===== */
    ...
    function syncProgramForm(){
      ...
      // under linear progression (user 2026-09-11, #54).
      if(threshold&&threshold.closest('.rule-field'))threshold.closest('.rule-field').hidden=scheme==='linear';
    ...
    function renderProgramWorkoutPage(program,workout){
      ...
      const listRows=rows.map(item=>{const s=savedExerciseSummary(item);return `<div class="picker-item saved-editor-row"><span><strong>${escapeHtml(s.name)}</strong><span>${escapeHtml(s.meta)}</span></span></div>`;}).join('');
    ...
    function openAddSavedToProgram(){
      ...
      const host=$('#addSavedToProgramList');
      ...
      host.querySelectorAll('[data-add-saved]').forEach(b=>b.addEventListener('click',()=>{
        ...
        const shell={uid:newProgramWorkoutUid(),name:t.name,sourceTemplateId:t.id,template:{name:t.name,exercises:cloneTemplateExercises(t.exercises||[])}};
        ...
        showToast(`Added "${t.name}" to the program.`);
      }));
    ...
    function cancelProgramEdit(){
      ...
      delete $('#createProgram').dataset.editing;$('#createProgram').textContent='Create active program';
      renderProgram();
    }
