    ...
    function renderBuiltInPrograms() {
      $('#builtInProgramList').innerHTML=strongLiftsWorkoutTemplates.map(template=>`<article class="program-template-card"><div><h3>${escapeHtml(template.name)}</h3><p>Straight-set strength template with a 5 lb progression step. Use it as a standalone workout or add it to a program you create.</p><div class="program-template-workouts"><div class="program-template-workout">${template.exercises.map(item=>{const ex=exercises.find(row=>row.id===item.exerciseId);return `${escapeHtml(ex?.name||item.exerciseId)} ${item.sets.length}×${item.progression?.max||5}`;}).join(' · ')}</div></div></div><button class="primary-button" type="button" data-start-built-in-template="${escapeHtml(template.id)}">Start workout</button></article>`).join('');
      document.querySelectorAll('[data-start-built-in-template]').forEach(button=>button.addEventListener('click',()=>{
        const template=workoutState.templates.find(row=>row.id===button.dataset.startBuiltInTemplate);if(!template)return;
        showWorkouts();workoutState.draft={name:template.name,date:localIsoDate(),programId:null,programWorkoutUid:null,editingId:null,sample:false,exercises:template.exercises.map(item=>({uid:uid('exercise'),exerciseId:item.exerciseId,tracking:item.tracking||item.progression?.mode||'reps',note:item.note||'',noteOpen:!!item.note,exerciseTags:[...(item.exerciseTags||[])],supersetId:item.supersetId||null,progression:item.progression?{...item.progression}:null,sets:item.sets.map(set=>({...newSet(),r:set.r??'',seconds:set.seconds??'',tags:[...(set.tags||[])]}))}))};prepareDraftProgression(workoutState.draft,{...progressionSetup,stallDetection:false});renderWorkoutScreen();
      }));
    }
    ...
    function suggestedProgramWorkout(program) {
      ...
      return ready.map((workout,index)=>{
        const logs=workoutState.completed.filter(row=>!row.sample&&row.programId===program.id&&row.programWorkoutUid===workout.uid);
    ...
    function renderArchivedPrograms() {
      ...
      $('#archivedProgramList').innerHTML=archived.map(program=>{
        ...
        return `<div class="archive-row-wrap"><div class="archive-row"><div><strong>${escapeHtml(program.name)} ${program.sample?'<span class="sample-label">Sample data</span>':''}</strong><br><span>${program.length} weeks · ${logs.length} logged workout${logs.length===1?'':'s'} · archived ${escapeHtml(formatLogDate(program.archivedAt))}</span></div>${program.sample?'':`<button class="small-button restore-program" type="button" data-program-id="${escapeHtml(program.id)}">Restore</button>`}</div><details class="archive-history"><summary>View program history${logs.length?` · ${logs.length} sessions`:''}</summary><div class="archive-history-list">${logs.length?logs.map(workout=>`<button class="archive-workout" type="button" data-archived-workout="${escapeHtml(workout.id)}"><strong>${escapeHtml(workout.name)}</strong><span>${escapeHtml(formatLogDate(workout.date))} · ${workout.exercises.reduce((sum,item)=>sum+item.sets.length,0)} sets</span></button>`).join(''):'<p class="section-note">No completed workouts are linked to this program.</p>'}</div></details></div>`;
      }).join('');
      document.querySelectorAll('.restore-program').forEach(button=>button.addEventListener('click',()=>{const index=workoutState.archivedPrograms.findIndex(p=>p.id===button.dataset.programId);if(index<0)return;const restored=workoutState.archivedPrograms.splice(index,1)[0];if(workoutState.activeProgram&&workoutState.activeProgram.id!==restored.id){workoutState.activeProgram.archivedAt=localIsoDate();workoutState.archivedPrograms.unshift(workoutState.activeProgram);}workoutState.activeProgram=restored;delete workoutState.activeProgram.archivedAt;renderProgram();renderDashboard();}));
    ...
    function startProgramWorkout(program, workout) {
      ...
      workoutState.draft={name:workout.name,date:localIsoDate(),programId:program.id,programWorkoutUid:workout.uid,editingId:null,exercises:workout.template.exercises.map(x=>{
        const base=x.progression||sampleProgressionProfiles[x.exerciseId]||{};
    ...
    function renderProgram() {
      ...
      $('#endProgram').addEventListener('click', () => {program.archivedAt=localIsoDate();workoutState.archivedPrograms.unshift(program);workoutState.activeProgram=null;$('#programName').value='';$('#programStartWeek').value='1';$('#programFocus').value='';renderProgram();renderDashboard();});
