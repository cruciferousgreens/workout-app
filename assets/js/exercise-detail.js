...
/* ===== module: exercise-detail.js ===== */
    ...
    function openExercise(id, push = true, returnTo) {
      ...
      function progressChartBlock(kicker,svg,points,valueLabel){
        ...
        return `<div class="chart-head-row"><p class="chart-kicker">${escapeHtml(kicker)}</p><div class="mini-segmented" role="group" aria-label="Progress metric">${seg}</div></div>`
          +`<div class="chart-tappable"><p class="chart-headline"><strong>${val}</strong><span>${when}</span></p>${svg}</div>`;
      }
