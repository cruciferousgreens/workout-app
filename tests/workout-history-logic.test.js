...
function fakeReviewDocument(){
  ...
  const staticText={'reviewSetsCancel':'Keep editing'};
  for(const id of ['reviewSetsCopy','reviewSetsComplete','reviewSetsFinishAnyway','reviewSetsDelete','reviewSetsCancel','reviewSetsDialog']){
    els['#'+id]={hidden:false,textContent:staticText[id]||'',className:'',opened:false,showModal(){this.opened=true;}};
  }
  return {els,querySelector:sel=>els[sel]||null,staticText};
}
const reviewDoc=globalThis.document;
function resetReviewDialog(){
  for(const [sel,el] of Object.entries(reviewDoc.els)){el.hidden=false;el.textContent=reviewDoc.staticText[sel.slice(1)]||'';el.className='';el.opened=false;}
}
...
describe('reviewSetsActions (user 2026-09-13, #262: exactly two actions when sets need review)',()=>{
  const partial=()=>mkSet({w:'100'}); /* weight entered, reps missing */
  const valid=()=>mkSet({w:'100',r:'8',complete:false});
  it("the user's all-partial scenario: Finish anyway + Keep editing",()=>{
  ...
  it('mixed empty + partial: the same two actions',()=>{
  ...
  it('all-empty: two actions here too — no middle option',()=>{
...
describe('reviewSetsCopy (user 2026-09-13, #262: the two-button copy)',()=>{
  ...
  it("the user's all-partial scenario: count + the Finish consequence",()=>{
    const draft=mkDraft([mkItem('squat',Array.from({length:8},partial))]);
    assert.equal(copyFor(draft),'8 sets are missing… Clicking Finish will delete your sets.');
  });
  it('all-empty uses the same copy',()=>{
    const draft=mkDraft([mkItem('squat',[mkSet(),mkSet()])]);
    assert.equal(copyFor(draft),'2 sets are missing… Clicking Finish will delete your sets.');
  });
  it('mixed empty + partial uses the same copy',()=>{
    const draft=mkDraft([mkItem('squat',[mkSet(),partial(),valid()])]);
    assert.equal(copyFor(draft),'2 sets are missing… Clicking Finish will delete your sets.');
  });
  it('singular: "1 set is missing"',()=>{
    const draft=mkDraft([mkItem('squat',[partial(),valid()])]);
    assert.equal(copyFor(draft),'1 set is missing… Clicking Finish will delete your sets.');
  });
  ...
  it('bad sets plus an empty exercise names both',()=>{
    ...
    const copy=copyFor(draft);
    assert.equal(copy,'1 set is missing… Clicking Finish will delete your sets. 1 exercise has no sets.');
  });
});

describe('showReviewSetsDialog (user 2026-09-13, #262: exactly two buttons)',()=>{
  const partial=()=>mkSet({w:'100'});
  const valid=()=>mkSet({w:'100',r:'8',complete:false});
  /* The visible actions, in DOM order. */
  const visibleActions=els=>{
    const order=['#reviewSetsComplete','#reviewSetsFinishAnyway','#reviewSetsDelete','#reviewSetsCancel'];
    return order.filter(sel=>!els[sel].hidden);
  };
  it("the user's all-partial scenario: primary 'Finish anyway' + secondary 'Keep editing'",()=>{
    ...
    assert.equal(els['#reviewSetsCancel'].className,'secondary-button','Keep editing is a real button');
    assert.equal(els['#reviewSetsCopy'].textContent,'8 sets are missing… Clicking Finish will delete your sets.');
  ...
  it('all-empty: the same two buttons',()=>{
    ...
    assert.equal(els['#reviewSetsCancel'].className,'secondary-button');
    assert.equal(els['#reviewSetsCopy'].textContent,'2 sets are missing… Clicking Finish will delete your sets.');
  });
  it('valid-but-unmarked: Mark all complete is primary, finish hidden, Keep editing a text link',()=>{
    const els=openReviewDialog(mkDraft([mkItem('squat',[valid(),valid()])]));
    assert.deepEqual(visibleActions(els),['#reviewSetsComplete','#reviewSetsCancel']);
  ...
  it('mixed: primary Finish anyway + secondary Keep editing',()=>{
    const els=openReviewDialog(mkDraft([mkItem('squat',[mkSet(),mkSet(),partial(),valid()])]));
    assert.deepEqual(visibleActions(els),['#reviewSetsFinishAnyway','#reviewSetsCancel']);
    assert.deepEqual(reviewPrimaryButtons(els),['#reviewSetsFinishAnyway']);
    assert.equal(els['#reviewSetsFinishAnyway'].textContent,'Finish anyway');
    assert.equal(els['#reviewSetsCancel'].className,'secondary-button');
    assert.equal(els['#reviewSetsCopy'].textContent,'3 sets are missing… Clicking Finish will delete your sets.');
  });
